...
class Ship(GSprite):
    ...
    def __init__ (self,x,y,width,height,source,frame):
        super().__init__(x=x,y=y,width=width,format=(2,4),height=height,source=source,frame=frame)

    # METHODS TO MOVE THE SHIP AND CHECK FOR COLLISIONS
    def _shipcollides(self, bolt):
        """
        Check to see if a bolt from an alien hits the ship

        Parameter: the laser bolts from an alien currently on screen
        Precondition: bolt is a Bolt object
        """
        if(bolt != None):
            list = [bolt.x, bolt.y]
            width = BOLT_WIDTH/2
            height = BOLT_HEIGHT/2
            if self.contains([list[0] + width, list[1]+height]):
                return True
            elif self.contains([list[0] - width, list[1]+height]):
                return True
            elif self.contains([list[0] + width, list[1]-height]):
                return True
            elif self.contains([list[0] + width, list[1]-height]):
    ...
    def _animate_ship(self,dt):
        '''
        Coroutine method to animate the ship blowing up
        ...
        while(steps < DEATH_SPEED):
            #print("here")
            dt = (yield)
            steps = steps + dt
            self.frame = int((steps/DEATH_SPEED)*7)
...
class Alien(GImage):
    ...
    def _aliencollides(self, bolt):
        """
        Check to see if a bolt from the ship hits an alien

        Parameter: the laser bolts from the ship currently on screen
        Precondition: bolt is a Bolt object
        """
        list = [bolt.x, bolt.y]
        width = BOLT_WIDTH/2
...
class Bolt(GRectangle):
    ...
    def getVel(self):
        '''
        This returns the self._velocity attribute
        '''
        return self._velocity

    # INITIALIZER TO SET THE VELOCITY
    def __init__(self,x,y,width,height, fillcolor, linecolor, where):
        super().__init__(x=x,y=y,width=width,height=height,fillcolor=fillcolor,linecolor=linecolor)
        if(where == True):
    ...
    def _isPlayerBolt(self):
        '''
        Check self._velocity to determine whether the bolt is a player bolt
        '''
